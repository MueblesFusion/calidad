            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {formData.area && (
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-64 overflow-auto border border-gray-300 rounded p-2 bg-white">
                    {defectosActuales.map((defecto) => (
                      <label key={defecto} className="flex items-center space-x-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={defectosSeleccionados.includes(defecto)}
                          onChange={() => handleToggleDefecto(defecto)}
                          className="h-4 w-4"
                        />
                        <span>{defecto}</span>
                      </label>
